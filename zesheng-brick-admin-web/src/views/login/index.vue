        ...
        >
          ...
          <div class="form-actions">
            <el-checkbox
              ...
              title="登录成功后会记住本次账号；勾选并登录则在本机同时保存密码，请勿在公共电脑上勾选"
            >
              记住密码
            </el-checkbox>
            <a
...
<script setup>
import { reactive, ref, toRefs, onMounted, computed, nextTick, watch } from "vue";
...
watch(rememberPassword, (checked) => {
  if (!checked) {
    clearStoredPasswordOnly();
    state.form.password = "";
  }
...
onMounted(() => {
  ...
  const saved = loadLastLoginState();
  state.form.username = saved.username;
  state.form.password = saved.password;
